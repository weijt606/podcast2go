...
1. 贴一个**链接**——文章、YouTube、Apple Podcasts、RSS 订阅源、或小宇宙（点**检测**确认能解析）。
2. 选一个**目标时长**（3 / 5 / 10 / 15 分钟）。
3. *（可选）* 展开**高级选项**：核心点 / 深度了解的点、语气视角、输出语言、单人或双人**模式**、以及**配音音色**（带 **▶ 试听**）。
4. 点**生成播客**，看五个步骤依次跑完（解析 → 提取 → 检索 → 脚本 → 合成）。
5. 播放器出现后：按播放、按**章节**跳转、看核心点与来源、读完整脚本，或**下载** mp3 留存；锁屏也能继续听（后台播放）。

**4 · 没在 `.env` 填 key？用界面面板。** 点顶部的设置面板，把 LLM Base URL / key / model 贴进去
（仅存于当前浏览器，手机上很方便）。留空则回退到 `backend/.env`。

> 想压缩播客/音频链接（Apple Podcasts、RSS、小宇宙、音频文件）？先 `pip install faster-whisper` 一次。
