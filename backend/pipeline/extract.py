"""Nebius LLM: long text -> ranked key points (map-reduce for long inputs)."""
...
async def extract_key_points(title: str, text: str, focus: str = "") -> dict:
    ...
    if not data.get("key_points"):
        # fallback so the pipeline never silently dies on a parse miss
        data = {
            "summary": title,
